...
export function QRCodeHandler({
  ...
  const [qrValue, setQrValue] = useState<string>(
    initialValue || sqlds.encode([Date.now(), Math.floor(Math.random() * 10000)])
  )
  const [qrCodeDataURL, setQrCodeDataURL] = useState<string>('')
  const [isScannerActive, setIsScannerActive] = useState<boolean>(mode === 'scan')
        ...
        <div className='qr-generator'>
            ...
            <div className='mb-4'>
              ...
              <div className='flex gap-3'>
                <input
                  type='text'
                  value={qrValue}
                  onChange={e => setQrValue((e.target as HTMLInputElement).value)}
          ...
          {qrCodeDataURL && (
            <div className='qr-display flex flex-col items-center'>
              <img src={qrCodeDataURL} alt='QR Code' className='select-none pointer-events-none' />
            </div>
          )}
        </div>
      )}
          ...
          <div className='w-full bg-white dark:bg-gray-800'>
            {mode !== 'scan' && (
              <div className='flex items-center justify-between mb-4'>
                <h3 className='text-xl font-bold text-gray-900 dark:text-white'>QR Code Scanner</h3>
                <Button
            ...
            {scanResult && (
              <div className='p-4 bg-gray-50 dark:bg-gray-700'>
                ...
                <p className='text-sm break-all text-gray-700 dark:text-gray-300'>{scanResult}</p>
