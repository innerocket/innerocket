...
export function QRCodePage({
  ...
  const handleConnect = () => {
    if (scannedValue && !connectedPeers.includes(scannedValue)) {
      ...
      showNotification(`Attempting to connect to peer ${scannedValue.substring(0, 8)}...`, 'info')
    }
  }
  ...
  return (
    <div className='qr-code-page max-w-4xl mx-auto py-6'>
      <h2 className='text-2xl font-bold text-gray-800 mb-6'>QR Code Connection</h2>
      ...
      <div className='bg-white rounded-lg p-6 mb-6'>
        ...
        <p className='mb-4 text-gray-600'>Scan someone else's QR code to connect with them.</p>
        <QRCodeHandler mode='scan' onScan={handleScan} />

        {scannedValue && (
          <div className='mt-6'>
            <div className='bg-gray-50 p-4 rounded-sm mb-4'>
              <h4 className='text-md font-medium text-gray-700'>Scanned Peer ID:</h4>
            ...
            {connectionStatus && <p className='mt-2 text-sm text-gray-600'>{connectionStatus}</p>}
          </div>
        )}
      </div>
    </div>
